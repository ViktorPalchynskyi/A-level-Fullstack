...
const store = createStore((state={status: '', payload: undefined, 
                                                error: undefined}, 
                           {type, status, payload, error, name}) =>
    (type === 'PROMISE') ? {status, payload, error} : state) //и этот name должен как-то учитываться в структуре хранилища
